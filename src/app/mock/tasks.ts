import { Partner } from "./partner";
import { Projects } from "./projects";

export interface Task {
    id: number;                                 // Eindeutige Ticket-ID

    name: string;                              // Titel oder Betreff des Tickets
    description: string;                        // Detaillierte Beschreibung des Problems oder der Anfrage
    creator: string;                            // Name oder ID des Erstellers des Tickets
    status: 'open' | 'in-progress' | 'closed';  // Aktueller Status des Tickets
    priority: 'low' | 'medium' | 'high';        // Priorität des Tickets
    category: string;                           // Kategorie des Tickets (z.B. Technisch, Kundensupport, Bug)
    isRunning: boolean;
    isPaused: boolean;
    assignedTo?: string;                        // Mitarbeiter, dem das Ticket zugewiesen ist (optional)
    createdAt: Date;                            // Datum und Uhrzeit, wann das Ticket erstellt wurde
    dueDate?: Date;                             // Optionales Fälligkeitsdatum für das Ticket
    attachments?: string[];                     // Liste der Dateipfade oder URLs für Anhänge (optional)

    createdFrom?: string;
    createdDate?: string;
    modifiedFrom?: string;
    modifiedDate?: string;
}

export const taskFieldNames: (keyof Task)[] = ['id', 'name', 'description', 'creator', 'status', 'priority',
    'category', 'assignedTo', 'createdAt', 'dueDate', 'attachments'
];

export const TASK_MOCK: Task[] = [
    {
        id: 0,
        name: 'Login-Probleme',
        description: 'Benutzer kann sich nicht in das System einloggen.',
        creator: 'Max Mustermann',
        status: 'open',
        priority: 'high',
        category: 'Technisch',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Lisa Schmidt',
        createdAt: new Date('2024-01-01T09:00:00'),
        dueDate: new Date('2024-01-05'),
        attachments: ['screenshot1.png'],
        createdFrom: 'Claudia Huber',
        createdDate: '14.07.2024',
        modifiedFrom: 'Thomas König',
        modifiedDate: '01.09.2024',
    },
    {
        id: 1,
        name: 'Falsche Rechnungsadresse',
        description: 'Die Rechnungsadresse ist im System falsch hinterlegt.',
        creator: 'Julia Fischer',
        status: 'in-progress',
        priority: 'medium',
        category: 'Kundensupport',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Peter Müller',
        createdAt: new Date('2024-01-02T08:30:00'),
        dueDate: new Date('2024-01-10'),
        attachments: [],
        createdFrom: 'Julian Maier',
        createdDate: '11.06.2024',
        modifiedFrom: 'Sabine Roth',
        modifiedDate: '17.08.2024',
    },
    {
        id: 2,
        name: 'Fehler bei der Produktseite',
        description: 'Auf der Produktseite wird ein Fehler beim Laden der Bilder angezeigt.',
        creator: 'Anna Bauer',
        status: 'open',
        priority: 'high',
        category: 'Bug',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Max Mustermann',
        createdAt: new Date('2024-01-03T10:00:00'),
        dueDate: new Date('2024-01-07'),
        attachments: ['error_log.txt'],
        createdFrom: 'Laura Becker',
        createdDate: '28.05.2024',
        modifiedFrom: 'Karl Zimmermann',
        modifiedDate: '04.09.2024',
    },
    {
        id: 3,
        name: 'Neues Feature für das Dashboard',
        description: 'Bitte ein neues Diagramm für das Benutzer-Dashboard hinzufügen.',
        creator: 'Sabine Neumann',
        status: 'closed',
        priority: 'low',
        category: 'Feature-Anfrage',
        isRunning: true,
        isPaused: false,
        assignedTo: 'Frank Meyer',
        createdAt: new Date('2024-01-04T11:00:00'),
        dueDate: new Date('2024-01-20'),
        attachments: [],
        createdFrom: 'Tobias Klein',
        createdDate: '02.07.2024',
        modifiedFrom: 'Daniela Hofmann',
        modifiedDate: '12.09.2024',
    },
    {
        id: 4,
        name: 'Performance-Probleme im Backend',
        description: 'Die Antwortzeiten des Backends sind stark verzögert.',
        creator: 'Jens Bauer',
        status: 'open',
        priority: 'high',
        category: 'Technisch',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Lisa Schmidt',
        createdAt: new Date('2024-01-05T09:30:00'),
        dueDate: new Date('2024-01-08'),
        attachments: ['performance_report.pdf'],
        createdFrom: 'Franziska Lehmann',
        createdDate: '30.06.2024',
        modifiedFrom: 'Lukas Brandt',
        modifiedDate: '22.08.2024',
    },
    {
        id: 5,
        name: 'E-Mail-Benachrichtigung funktioniert nicht',
        description: 'Benachrichtigungen werden nicht an die Benutzer gesendet.',
        creator: 'Laura Bergmann',
        status: 'in-progress',
        priority: 'medium',
        category: 'Bug',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Peter Müller',
        createdAt: new Date('2024-01-06T08:00:00'),
        dueDate: new Date('2024-01-12'),
        attachments: [],
        createdFrom: 'Miriam König',
        createdDate: '05.07.2024',
        modifiedFrom: 'Stefan Schröder',
        modifiedDate: '10.09.2024',
    },
    {
        id: 6,
        name: 'Neues Design für die Startseite',
        description: 'Entwicklung eines neuen Designs für die Startseite.',
        creator: 'Martin Schulz',
        status: 'open',
        priority: 'low',
        category: 'Feature-Anfrage',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Anna Bauer',
        createdAt: new Date('2024-01-07T10:30:00'),
        dueDate: new Date('2024-02-01'),
        attachments: ['design_mockup.pdf'],
        createdFrom: 'Nina Weber',
        createdDate: '17.06.2024',
        modifiedFrom: 'Jan Böhm',
        modifiedDate: '07.08.2024',
    },
    {
        id: 7,
        name: 'Datenbank-Migration',
        description: 'Migration der Datenbank auf einen neuen Server.',
        creator: 'Oliver Becker',
        status: 'open',
        priority: 'high',
        category: 'Technisch',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Jens Bauer',
        createdAt: new Date('2024-01-08T09:00:00'),
        dueDate: new Date('2024-01-15'),
        attachments: [],
        createdFrom: 'Ralf Wagner',
        createdDate: '03.07.2024',
        modifiedFrom: 'Katharina Scholz',
        modifiedDate: '25.08.2024',
    },
    {
        id: 8,
        name: 'Problem mit Zahlungsabwicklung',
        description: 'Zahlungen werden nicht korrekt verarbeitet.',
        creator: 'Heike Fischer',
        status: 'in-progress',
        priority: 'high',
        category: 'Kundensupport',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Frank Meyer',
        createdAt: new Date('2024-01-09T11:00:00'),
        dueDate: new Date('2024-01-13'),
        attachments: ['payment_issue_report.pdf'],
        createdFrom: 'Lisa Hoffmann',
        createdDate: '12.07.2024',
        modifiedFrom: 'Alexander Koch',
        modifiedDate: '29.08.2024',
    },
    {
        id: 9,
        name: 'Anpassung der Datenschutzerklärung',
        description: 'Aktualisierung der Datenschutzerklärung aufgrund neuer Vorschriften.',
        creator: 'Lena Wagner',
        status: 'closed',
        priority: 'medium',
        category: 'Rechtlich',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Lisa Schmidt',
        createdAt: new Date('2024-01-10T12:00:00'),
        dueDate: new Date('2024-01-30'),
        attachments: ['new_privacy_policy.pdf'],
        createdFrom: 'Tim Jansen',
        createdDate: '21.06.2024',
        modifiedFrom: 'Eva Schäfer',
        modifiedDate: '03.09.2024',
    },
    {
        id: 10,
        name: 'Überprüfung der Datenschutzerklärung durch Rechtsabteilung',
        description: 'Finale Überprüfung der aktualisierten Datenschutzerklärung durch die Rechtsabteilung.',
        creator: 'Lena Wagner',
        status: 'open',
        priority: 'high',
        category: 'Rechtlich',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Maximilian Bauer',
        createdAt: new Date('2024-02-01T09:00:00'),
        dueDate: new Date('2024-02-15'),
        attachments: ['privacy_policy_review.pdf'],
        createdFrom: 'Monika Krüger',
        createdDate: '07.07.2024',
        modifiedFrom: 'Björn Peters',
        modifiedDate: '20.09.2024',
    },
    {
        id: 11,
        name: 'Einbindung der neuen Datenschutzerklärung auf der Website',
        description: 'Die überarbeitete Datenschutzerklärung soll auf der Unternehmenswebsite veröffentlicht werden.',
        creator: 'Lena Wagner',
        status: 'in-progress',
        priority: 'medium',
        category: 'IT',
        isRunning: true,
        isPaused: false,
        assignedTo: 'Jonas Meier',
        createdAt: new Date('2024-02-16T08:30:00'),
        dueDate: new Date('2024-02-20'),
        attachments: ['datenschutzerklaerung_final.pdf'],
        createdFrom: 'Florian Berger',
        createdDate: '10.07.2024',
        modifiedFrom: 'Carina Frank',
        modifiedDate: '14.09.2024',
    },
    {
        id: 12,
        name: 'Schulung zum Umgang mit neuen Datenschutzrichtlinien',
        description: 'Interne Schulung für Mitarbeiter zur Umsetzung der neuen Datenschutzvorgaben.',
        creator: 'Lena Wagner',
        status: 'open',
        priority: 'low',
        category: 'HR',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Sophie Keller',
        createdAt: new Date('2024-03-01T11:00:00'),
        dueDate: new Date('2024-03-15'),
        attachments: ['schulungsunterlagen.pdf'],
        createdFrom: 'Vanessa Arnold',
        createdDate: '23.06.2024',
        modifiedFrom: 'Michael Lorenz',
        modifiedDate: '05.09.2024',
    },
    {
        id: 13,
        name: 'Integration neuer Zahlungsanbieter',
        description: 'Einbindung von PayPal als zusätzliche Zahlungsmöglichkeit.',
        creator: 'Svenja Richter',
        status: 'open',
        priority: 'medium',
        category: 'Technisch',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Peter Müller',
        createdAt: new Date('2024-03-05T09:30:00'),
        dueDate: new Date('2024-03-20'),
        attachments: ['paypal_api_docs.pdf'],
        createdFrom: 'Nina Weber',
        createdDate: '02.07.2024',
        modifiedFrom: 'Frank Meyer',
        modifiedDate: '12.09.2024',
    },
    {
        id: 14,
        name: 'Kundenzufriedenheitsumfrage vorbereiten',
        description: 'Erstellung und Versand einer Umfrage zur Kundenzufriedenheit.',
        creator: 'Clara Hoffmann',
        status: 'in-progress',
        priority: 'low',
        category: 'Marketing',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Anna Bauer',
        createdAt: new Date('2024-03-10T10:00:00'),
        dueDate: new Date('2024-03-25'),
        attachments: [],
        createdFrom: 'Julia Fischer',
        createdDate: '15.07.2024',
        modifiedFrom: 'Max Mustermann',
        modifiedDate: '17.09.2024',
    },
    {
        id: 15,
        name: 'Sicherheitsupdate einspielen',
        description: 'Installation des neuesten Sicherheitsupdates auf allen Servern.',
        creator: 'Tobias Klein',
        status: 'open',
        priority: 'high',
        category: 'IT',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Jens Bauer',
        createdAt: new Date('2024-03-12T08:00:00'),
        dueDate: new Date('2024-03-15'),
        attachments: ['update_notes.pdf'],
        createdFrom: 'Sabine Neumann',
        createdDate: '06.07.2024',
        modifiedFrom: 'Laura Bergmann',
        modifiedDate: '01.09.2024',
    },
    {
        id: 16,
        name: 'Fehlerhafte Bestellbestätigung',
        description: 'Kunden erhalten keine Bestellbestätigung per E-Mail.',
        creator: 'Peter Müller',
        status: 'in-progress',
        priority: 'medium',
        category: 'Kundensupport',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Lisa Schmidt',
        createdAt: new Date('2024-03-14T11:30:00'),
        dueDate: new Date('2024-03-18'),
        attachments: [],
        createdFrom: 'Martin Schulz',
        createdDate: '28.06.2024',
        modifiedFrom: 'Jens Bauer',
        modifiedDate: '09.09.2024',
    },
    {
        id: 17,
        name: 'Onboarding neuer Mitarbeiter',
        description: 'Durchführung des Onboardings für neue Teammitglieder.',
        creator: 'Sophie Keller',
        status: 'open',
        priority: 'low',
        category: 'HR',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Lena Wagner',
        createdAt: new Date('2024-03-16T09:00:00'),
        dueDate: new Date('2024-03-22'),
        attachments: ['onboarding_checklist.pdf'],
        createdFrom: 'Laura Becker',
        createdDate: '03.07.2024',
        modifiedFrom: 'Oliver Becker',
        modifiedDate: '12.09.2024',
    },
    {
        id: 18,
        name: 'SEO-Optimierung der Produktseiten',
        description: 'Überarbeitung der Produktseiten für bessere Suchmaschinenplatzierung.',
        creator: 'Jan Böhm',
        status: 'open',
        priority: 'medium',
        category: 'Marketing',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Sabine Roth',
        createdAt: new Date('2024-03-18T14:00:00'),
        dueDate: new Date('2024-03-28'),
        attachments: ['seo_report.pdf'],
        createdFrom: 'Clara Hoffmann',
        createdDate: '12.07.2024',
        modifiedFrom: 'Julia Fischer',
        modifiedDate: '19.09.2024',
    },
    {
        id: 19,
        name: 'Test der mobilen App',
        description: 'Durchführung von Usability-Tests für die neue mobile App.',
        creator: 'Björn Peters',
        status: 'in-progress',
        priority: 'high',
        category: 'QA',
        isRunning: true,
        isPaused: false,
        assignedTo: 'Anna Bauer',
        createdAt: new Date('2024-03-20T13:30:00'),
        dueDate: new Date('2024-03-27'),
        attachments: ['testbericht.pdf'],
        createdFrom: 'Frank Meyer',
        createdDate: '21.06.2024',
        modifiedFrom: 'Jonas Meier',
        modifiedDate: '14.09.2024',
    },
    {
        id: 20,
        name: 'Update der FAQ-Seite',
        description: 'Ergänzung neuer Fragen und Antworten auf der FAQ-Seite.',
        creator: 'Carina Frank',
        status: 'closed',
        priority: 'low',
        category: 'Content',
        isRunning: false,
        isPaused: false,
        assignedTo: 'Lisa Schmidt',
        createdAt: new Date('2024-03-22T10:15:00'),
        dueDate: new Date('2024-03-24'),
        attachments: [],
        createdFrom: 'Heike Fischer',
        createdDate: '11.07.2024',
        modifiedFrom: 'Monika Krüger',
        modifiedDate: '17.09.2024',
    },

];
